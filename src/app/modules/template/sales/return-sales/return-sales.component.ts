...
export class ReturnSalesComponent implements OnInit {
  ...
  addSalesOrderFormBuilder() {
    this.addSalesOrder = this.fb.group({
      ...
      subTotal: [""],
      sgstAmount: ['', [Validators.required, Validators.pattern(/^[1-9]\d*(\.\d+)?$/)]],
      cgstAmount: ['', [Validators.required, Validators.pattern(/^[1-9]\d*(\.\d+)?$/)]],
      totalNetAmount: [""],
      salesOrderList: ""
    });
  }
  ...
  addSalesOrderItemsIntoList(salesOrderListData) {
    for (let index = 0; index < salesOrderListData.object.length; index++) {
      this.salesOrder = {
        productType: salesOrderListData.object[index].stockItemId.productType.categoryName,
        productName: salesOrderListData.object[index].stockItemId.productName.productName,
        manufacturer: salesOrderListData.object[index].stockItemId.productName.manufacturer.manufacturerName,
        ...
        manufactureDate: salesOrderListData.object[index].stockItemId.manufactureDate,
  ...
  validateSalesOrderDetails(i: number) {
    ...
    this.salesOrderArray.every((object, index) => {
      ...
      if (productTypeRowFlag && productNameFlag && quantityRowFlag && unitPriceFlag) {
  ...
  productTypeRow(productTypeValue: any, i: number) {
    if (productTypeValue != "") {
      ...
      this.getProductListUsingCategoryId(productTypeValue.productType.categoryId);
  ...
  productNameRow(productNameValue: any, i: number) {
    if (productNameValue != "") {
      ...
      this.salesOrderArray[i].manufacturer = productNameValue.productName.manufacturer.manufacturerName;
      ...
      this.salesOrderArray[i].manufactureDate = productNameValue.manufactureDate;
